...
import { ConversationStrategy, JerryMessage, StrategyContext } from '../../shared/types'
...
export class StrategyPlannerService {
  decide(ctx: StrategyContext): ConversationStrategy {
    ...
    if (extractedData && Object.keys(extractedData).length > 0) {
      ...
      return { type: 'confirm_and_probe', targetField: topKey, confirmedData: extractedData }
    }

    return { type: 'strategic_ask', targetField: this.pickNextField(missingFields, session.messages) }
  }
  ...
  private detectFrustration(messages: JerryMessage[]): boolean {
    ...
    return recentMessages.some(
      (msg) =>
        msg.role === 'user' &&
        FRUSTRATION_KEYWORDS.some((kw) => msg.content.toLowerCase().includes(kw)),
    )
  }

  private pickNextField(missingFields: string[], messages: JerryMessage[]): string | undefined {
  ...
  private getLastAskedField(missingFields: string[], messages: JerryMessage[]): string | undefined {
