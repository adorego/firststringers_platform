...
<<<<<<< Updated upstream
import { OnboardingWizard } from "@/components/recruiter/OnboardingWizard";
...
export default function BillyLandingPage() {
  ...
  const recruiterName = session?.user?.name ?? "there";

  const [input, setInput] = useState("");
  const [isCreating, setIsCreating] = useState(false);
<<<<<<< Updated upstream
  const [onboardingStatus, setOnboardingStatus] = useState<"loading" | "needed" | "done">(
    "loading",
  );
  ...
  useEffect(() => {
    ...
    api
      .getRecruiterProfile()
      .then((profile) => {
        setOnboardingStatus(profile.onboardingCompleted ? "done" : "needed");
      })
      .catch(() => {
        // If we can't check, skip onboarding to not block the user
        setOnboardingStatus("done");
      });
  }, [session]);

  const handleOnboardingComplete = (suggestedSearches: string[]) => {
    if (suggestedSearches.length > 0) setSuggestions(suggestedSearches);
    setOnboardingStatus("done");
  };
=======
...
>>>>>>> Stashed changes
  ...
  return (
    <>
      {/* Onboarding wizard — full-screen overlay, auto-shows on first login */}
      ...
      {/* Normal landing — shown after onboarding or if already completed */}
      <div className="flex h-full flex-col items-center justify-center bg-[#F5F0EB] px-8 pb-16">
        {onboardingStatus === "loading" ? (
          <div className="flex items-center gap-2 text-sm text-[#ADA8A5]">
