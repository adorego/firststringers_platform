...
const jerryIcon = (
  <div className="flex h-6 w-6 items-center justify-center rounded-full bg-fs-text">
    <div className="h-1.5 w-1.5 rounded-full bg-fs-bg" />
  </div>
);
...
export function BottomNav() {
  ...
  return (
    <nav className="flex h-16 items-center border-t border-fs-surface-alt bg-fs-bg">
      {items.map((item) => {
        ...
        return (
          <Link
            ...
            className={`relative flex flex-1 flex-col items-center gap-1 py-2 transition-colors ${
              active
                ? "rounded-xl bg-fs-surface-alt mx-1 text-fs-text"
                : "text-fs-text-muted hover:text-fs-text-muted"
            }`}
          >
            <div className="relative">
              {active ? item.activeIcon : item.icon}
              {item.badge && (
                <span className="absolute -right-1 -top-1 h-2 w-2 rounded-full bg-fs-text" />
              )}
            </div>
