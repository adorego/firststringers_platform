...
`apps/api/src/modules/billy/billy.worker.ts` (Billy). A draft spec may first use a documentation PR
to collect product and technical approval. Once marked implementation-ready, the linked runtime PR
must update the prompts and gated behavior (e.g. FS-CS-005 representation lifecycle) against that
approved version before deployment.
...
5. **Implement** — a linked runtime PR updates the derived prompts/behavior against the approved
   spec version and passes the required evaluations.
6. **Deploy** — the platform and ChatGPT "Intelligence Core" sandbox use the same approved version.
